...
@test "prioritization: add task with valid importance and urgency" {
    brex tmp_vault task add --title "Test task" --importance 4 --urgency 3
...
@test "prioritization: validate importance bounds" {
    run brex tmp_vault task add --title "Test task" --importance 0
    [ "$status" -ne 0 ]
    [[ "$output" =~ "Importance must be an integer between 1 and 5" ]]

    run brex tmp_vault task add --title "Test task" --importance 6
    [ "$status" -ne 0 ]
    [[ "$output" =~ "Importance must be an integer between 1 and 5" ]]
}

@test "prioritization: validate urgency bounds" {
    run brex tmp_vault task add --title "Test task" --urgency 0
    [ "$status" -ne 0 ]
    [[ "$output" =~ "Urgency must be an integer between 1 and 5" ]]

    run brex tmp_vault task add --title "Test task" --urgency 6
    [ "$status" -ne 0 ]
    [[ "$output" =~ "Urgency must be an integer between 1 and 5" ]]
}

@test "prioritization: optional due date defaults to NULL" {
    brex tmp_vault task add --title "Test task"
...
@test "prioritization: dynamic urgency boosts" {
    ...
    brex tmp_vault task add --title "Overdue task" --due_to "2025-01-01 12:00:00" --importance 4 --urgency 1

    # Task due in far future -> manual urgency 3
    LATE_DATE=$(date -d "+10 days" "+%Y-%m-%d %H:%M:%S" 2>/dev/null || date -v+10d "+%Y-%m-%d %H:%M:%S")
    brex tmp_vault task add --title "Far future task" --due_to "$LATE_DATE" --importance 4 --urgency 3
...
@test "prioritization: sorting order Q1 -> Q3 -> Q2 -> Q4 -> NULL deadlines" {
    ...
    brex tmp_vault task add --title "NULL deadline task"

    # 2. Q4 task with far due date (Importance 1, Urgency 1)
    LATE_DATE=$(date -d "+20 days" "+%Y-%m-%d %H:%M:%S" 2>/dev/null || date -v+20d "+%Y-%m-%d %H:%M:%S")
    brex tmp_vault task add --title "Q4 task" --due_to "$LATE_DATE" --importance 1 --urgency 1

    # 3. Q2 task (Importance 5, Urgency 2)
    brex tmp_vault task add --title "Q2 task" --due_to "$LATE_DATE" --importance 5 --urgency 2

    # 4. Q1 task (overdue -> active urgency 5, Importance 5)
    brex tmp_vault task add --title "Q1 task" --due_to "2025-01-01 12:00:00" --importance 5 --urgency 1

    # 5. Q3 task (overdue -> active urgency 5, Importance 2)
    brex tmp_vault task add --title "Q3 task" --due_to "2025-01-01 12:00:00" --importance 2 --urgency 1
...
@test "prioritization: hide due_to column via config setting" {
    ...
    brex tmp_vault task add --title "Default task"
    run brex tmp_vault task list
    [ "$status" -eq 0 ]
    [[ "$output" =~ "due_to" ]]
...
@test "prioritization: update priority rankings" {
    brex tmp_vault task add --title "Rank update task" --importance 1 --urgency 1

    # Extract task ID
    TASK_ID=$(sqlite3 tmp_vault.db "SELECT id FROM notes WHERE title='Rank update task';")
    [ -n "$TASK_ID" ]
    ...
    IMP=$(sqlite3 tmp_vault.db "SELECT importance FROM tasks WHERE item_id='$TASK_ID';")
    URG=$(sqlite3 tmp_vault.db "SELECT urgency FROM tasks WHERE item_id='$TASK_ID';")
    [ "$IMP" -eq 5 ]
    [ "$URG" -eq 4 ]
